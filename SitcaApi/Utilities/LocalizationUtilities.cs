...
public static class LocalizationUtilities
{
    ...
    private static readonly Dictionary<int, (string Es, string En)> StatusTranslations = new()
    {
        ...
        { Constants.ProcessStatus.ForConsulting, ("1 - Para Asesorar", "1 - For consulting") },
        {
            ...
            ("2 - Asesoria en Proceso", "2 - Consultancy underway")
        },
        {
            Constants.ProcessStatus.ConsultancyCompleted,
            ("3 - Asesoria Finalizada", "3 - Consultancy completed")
        },
        { Constants.ProcessStatus.ForAuditing, ("4 - Para Auditar", "4 - For auditing") },
        {
            Constants.ProcessStatus.AuditingUnderway,
            ("5 - Auditoria en Proceso", "5 - Auditing underway")
        },
        {
            Constants.ProcessStatus.AuditCompleted,
            ("6 - Auditoria Finalizada", "6 - Audit completed")
        },
        {
            Constants.ProcessStatus.UnderCCTReview,
            ("7 - En revisión de CTC", "7 - Under CCT Review")
        },
        { Constants.ProcessStatus.Completed, ("8 - Finalizado", "8 - Completed") },
    };

    public static readonly Dictionary<string, (string Es, string En)> DistintivoStatus = new()
    {
        { Constants.Distintivos.Rojo, ("Distintivo Rojo", "Red Badge") },
    ...
    public static int? GetStatusIdExact(string statusText, string lang)
    {
        ...
        return StatusTranslations
            .FirstOrDefault(x =>
                ...
                || (
                    lang == "en"
                    && x.Value.En.Equals(statusText, StringComparison.OrdinalIgnoreCase)
                )
            )
            .Key;
    }
    ...
    public static int? GetStatusId(string statusText, string lang)
    {
        ...
        var normalizedSearchText = statusText
            .Trim()
            .Replace("- ", "")
        ...
        return StatusTranslations
            .FirstOrDefault(x =>
                (
                    lang == "es"
                    && x.Value.Es.EndsWith(normalizedSearchText, StringComparison.OrdinalIgnoreCase)
                )
                || (
                    lang == "en"
                    && x.Value.En.EndsWith(normalizedSearchText, StringComparison.OrdinalIgnoreCase)
                )
            )
            .Key;
    }
