...
namespace Sitca.Controllers
{
  ...
  public class AuthController : ControllerBase
  {
    ...
    public async Task<IActionResult> RenewToken()
    {
      ...
      {
        _logger.LogError(ex, "Error renewing token for user {UserId}", User.FindFirst(ClaimTypes.NameIdentifier)?.Value);
        return StatusCode(500, new { error = "An error occurred while renewing the token" });
      }
    }
