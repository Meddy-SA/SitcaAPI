...
namespace Sitca.DataAccess.Data.Repository
{
    class CertificacionRepository : Repository<ProcesoCertificacion>, ICertificacionRepository
    {
        ...
        public async Task<Result<bool>> UpdateNumeroExpAsync(CertificacionDetailsVm data)
        {
                ...
                return await strategy.ExecuteAsync(async () =>
                {
                    ...
                    .FirstOrDefaultAsync(c => c.Id == data.Id);
                        ...
                        if (certificacion == null)
                            return Result<bool>.Failure($"No se encontró la certificación con ID {data.Id}");
            ...
            catch (Exception ex)
            {
                _logger.LogError(ex,
                    ...
                    data.Id,
                    ex.Message);
                return Result<bool>.Failure($"Error al actualizar número de expediente: {ex.Message}");
            }
        }
        ...
        public async Task<Result<int>> AsignaAuditorAsync(AsignaAuditoriaVm data, string language = "es")
        {
            // NOTE: Agregar tipologia
            try
            {
                var proceso = await _db.ProcesoCertificacion
                  .FirstOrDefaultAsync(s => s.EmpresaId == data.EmpresaId && s.FechaFinalizacion == null);
            ...
            {
                _logger.LogError(ex, "Error al asignar auditor {AuditorId} a empresa {EmpresaId}",
                  data.AuditorId, data.EmpresaId);
                return Result<int>.Failure("Error al asignar auditor");
            }
        }

        public async Task<Result<CuestionarioDetailsMinVm>> GenerarCuestionarioAsync(CuestionarioCreateVm data, string userGenerador, string role)
        {
            if (data == null)
            ...
            try
            {
                ...
                if (empresa == null) return Result<CuestionarioDetailsMinVm>.Failure("La empresa no existe");

                var proceso = await _db.ProcesoCertificacion.FindAsync(data.CertificacionId);
                if (proceso == null) return Result<CuestionarioDetailsMinVm>.Failure("La certificación no existe");
                ...
                switch (role)
                {
                    case ConstantRoles.Asesor:
                        ...
                        proceso.Status = StatusConstants.GetLocalizedStatus(ProcessStatus.ConsultancyUnderway, "es");
                        break;

                    case ConstantRoles.Auditor:
                        empresa.Estado = ProcessStatus.AuditingUnderway;
                        proceso.Status = StatusConstants.GetLocalizedStatus(ProcessStatus.AuditingUnderway, "es");
                        break;
                }
            ...
            {
                _logger.LogError(ex, "Error generando cuestionario para empresa {EmpresaId}", data.EmpresaId);
                return Result<CuestionarioDetailsMinVm>.Failure($"Error generando cuestionario: {ex.Message}");
            }
        }

        public async Task<Result<int>> ComenzarProcesoAsync(CertificacionVm data, string userGenerador)
        {
            try
            {
                if (data == null)
                ...
                return await strategy.ExecuteAsync(async () =>
                {
                    ...
                    var otrosProcesos = await _db.ProcesoCertificacion
                .AnyAsync(s => s.EmpresaId == data.EmpresaId);
            ...
            catch (Exception ex)
            {
                _logger.LogError(ex,
                    ...
                    data.EmpresaId,
                    ex.Message);
                return Result<int>.Failure($"Error al iniciar el proceso de certificación: {ex.Message}");
            }
        }
        ...
        public async Task<Result<bool>> CambiarAuditorAsync(CambioAuditor data)
        {
            ...
            try
            {
                return await strategy.ExecuteAsync(async () =>
                {
                    ...
                    {
                        var proceso = await _db.ProcesoCertificacion
                    .FindAsync(data.idProceso);

                        if (proceso == null)
                            return Result<bool>.Failure($"No se encontró el proceso con ID {data.idProceso}");

                        // Obtener todos los cuestionarios que necesitan actualización
                        var cuestionarios = await _db.Cuestionario
                    .Where(s => s.ProcesoCertificacionId == proceso.Id
                               && !s.Prueba
            ...
            catch (Exception ex)
            {
                ...
                return Result<bool>.Failure($"Error al cambiar {(data.auditor ? "auditor" : "asesor")}: {ex.Message}");
            }
        }
