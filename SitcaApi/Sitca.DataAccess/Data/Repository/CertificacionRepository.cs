...
namespace Sitca.DataAccess.Data.Repository
{
    class CertificacionRepository : Repository<ProcesoCertificacion>, ICertificacionRepository
    {
        ...
        public CertificacionRepository(ApplicationDbContext db, IConfiguration configuration) : base(db)
        {
            _db = db;
        ...
        public async Task<CuestionarioDetailsVm> GetCuestionario(int id, ApplicationUser user, string role)
        {
            ...
            var respuestas = _db.CuestionarioItem.Include(s =>s.Archivos).Where(s => s.CuestionarioId == id).ToList();
            //var proceso = _db.ProcesoCertificacion.OrderByDescending(x => x.Id).FirstOrDefault(s => s.EmpresaId == cuestionario.IdEmpresa);
            ...
            var modulos = _db.Modulo.Include("Secciones").Where(s => s.TipologiaId == cuestionario.IdTipologia || s.TipologiaId == null).OrderBy(m => m.Orden);
            

            if(_config["Settings:bioseguridad"] == "false")
            {
                //si no esta activo el modulo de bioseguridad, quitarlo del cuestionario
                modulos = modulos.Where(s => s.Id < 11).OrderBy(m => m.Orden);
            }
            ...
            foreach (var modulo in modulos)
            {
                ...
                {
                    #region SECCIONES
                    foreach (var seccion in modulo.Secciones.Where(s =>s.TipologiaId == cuestionario.IdTipologia || s.TipologiaId == null).OrderBy(x => x.Orden))
                    {
                        ...
                        #region SUBTITULOS
                        var subtitulos = _db.SubtituloSeccion.Where(s => s.SeccionModuloId == seccion.Id);
                        if (subtitulos.Any())
                        {
                            foreach (var subtitulo in subtitulos)
                            {
                                ...
                                var preguntasSubs = GetPreguntas(modulo.Id, seccion.Id, subtitulo.Id, user.Lenguage);
                        ...
                        var preguntas = GetPreguntas(modulo.Id, seccion.Id, none, user.Lenguage);
                        if (preguntas.Any())
                        {
                ...
                {
                    var preguntas = GetPreguntas(modulo.Id, none, none, user.Lenguage);
                    if (preguntas.Any())
                    {
                        preguntasC.AddRange(preguntas);
                    }
        ...
        public List<CuestionarioItemVm> GetPreguntas(int idModulo, int? idSeccion, int? idSubtitulo, string lang = "es")
        {
            if (idSubtitulo > 0)
            {
                return _db.Pregunta.Where(s => s.SubtituloSeccionId == idSubtitulo).Select(x => new CuestionarioItemVm
                {
                    Id = x.Id,
                    Nomenclatura = x.Nomenclatura,
                    Text = lang == "es"? x.Texto:x.Text,
                    Order = Int32.Parse(x.Orden),
                    Result = 0,
                    Type = "pregunta",
                    NoAplica = x.NoAplica,
            ...
            if (idSeccion > 0)
            {
                return _db.Pregunta.Where(s => s.SeccionModuloId == idSeccion && s.SubtituloSeccionId == null).Select(x => new CuestionarioItemVm
                {
                    Id = x.Id,
                    Nomenclatura = x.Nomenclatura,
                    Text = lang == "es" ? x.Texto:x.Text,
            ...
            return _db.Pregunta.Where(s => s.ModuloId == idModulo && s.SeccionModuloId == null && s.SubtituloSeccionId == null).Select(x => new CuestionarioItemVm
            {
                Id = x.Id,
                Nomenclatura = x.Nomenclatura,
                Text = lang == "es" ? x.Texto : x.Text,
        ...
        public async Task<bool> IsCuestionarioCompleto(CuestionarioDetailsVm data)
        {
            var items = data.Modulos.SelectMany(x => x.Items);
            var incompleto = items.Any(s => s.Type == "pregunta" && (s.Result == null || s.Result == 0 ));
        ...
        public async Task<List<CommonVm>> GetStatusList(string lang)
        {
            //0 - Inicial
            //1 - Para Asesorar
            //2 - Asesoria en Proceso
            ...
            if (lang == "es")
            {
                statuses.Add(new CommonVm { id = 0, name = "Inicial" });
                statuses.Add(new CommonVm { id = 1, name = "Para Asesorar" });
                statuses.Add(new CommonVm { id = 2, name = "Asesoria en Proceso" });
                statuses.Add(new CommonVm { id = 3, name = "Asesoria Finalizada" });
                statuses.Add(new CommonVm { id = 4, name = "Para Auditar" });
                statuses.Add(new CommonVm { id = 5, name = "Auditoria en Proceso" });
                statuses.Add(new CommonVm { id = 6, name = "Auditoria Finalizada" });
                statuses.Add(new CommonVm { id = 7, name = "En revisión de CTC" });
                statuses.Add(new CommonVm { id = 8, name = "Finalizado" });

                return statuses;
            }

            statuses.Add(new CommonVm { id = 0, name = "Initial" });
            statuses.Add(new CommonVm { id = 1, name = "To be Advised" });
            statuses.Add(new CommonVm { id = 2, name = "In Advising Process" });
            statuses.Add(new CommonVm { id = 3, name = "Advising Finalized" });
            statuses.Add(new CommonVm { id = 4, name = "To be Audited" });
            statuses.Add(new CommonVm { id = 5, name = "In Auditing Process" });
            statuses.Add(new CommonVm { id = 6, name = "Auditing Finalized" });
            statuses.Add(new CommonVm { id = 7, name = "Under CTC Review" });
            statuses.Add(new CommonVm { id = 8, name = "Ended" });
