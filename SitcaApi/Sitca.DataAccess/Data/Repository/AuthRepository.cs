...
public class AuthRepository : IAuthRepository
{
      ...
      ILogger<AuthRepository> logger)
  {
    ...
    _logger = logger;
    _url = _config["ExternalServices:WebUrl"] ?? throw new InvalidOperationException("WebUrl configuration is missing");
  }
  ...
  public async Task<AuthResult> LoginAsync(LoginDTO loginDto)
  {
    try
    {
      ...
      if (!validPassword)
      {
        ...
        return AuthResult.Failed(GetLocalizedMessage("InvalidCredentials", loginDto.Language));
      }
  ...
  public async Task<AuthResult> RegisterAsync(RegisterDTO register)
  {
    try
    {
      ...
      if (!result.Succeeded)
      {
        return AuthResult.Failed(GetLocalizedMessage("RegistrationError", register.Language) +
  ...
  public async Task<AuthResult> RenewTokenAsync(string userId)
  {
    try
    {
      ...
      {
        return AuthResult.Failed(GetLocalizedMessage("UserNotFound", user?.Lenguage ?? "en"));
      }
  ...
  private async Task<(ApplicationUser User, string Error)> GetAndValidateUserAsync(string email, string language)
  {
    try
    {
      ...
      if (user == null)
      {
        _logger.LogWarning("FindByEmailAsync retornó null para un usuario que existe en DB");
        return (null, GetLocalizedMessage("InvalidCredentials", language));
      }
      if (!user.Active)
      {
  ...
  private async Task<IdentityResult> CreateUserAsync(ApplicationUser user, RegisterDTO register) =>
  ...
  private async Task SendWelcomeEmailAsync(ApplicationUser user, string language)
  {
    try
    {
      ...
      var emailContent = await _viewRenderService.RenderToStringAsync("WelcomeMail", mailData);
  ...
  private string GetLocalizedMessage(string messageKey, string language) => messageKey switch
  {
    "InvalidCredentials" => language == "en" ? "Invalid credentials or inactive user" : "Credenciales inválidas o usuario inactivo",
    "EmailExists" => language == "en" ? "Email already registered" : "El correo electrónico ya está registrado",
    "RegistrationError" => language == "en" ? "Failed to create user: " : "Error al crear usuario: ",
    "UserNotFound" => language == "en" ? "User not found or inactive" : "Usuario no encontrado o inactivo",
    "GeneralError" => language == "en" ? "An unexpected error occurred" : "Ocurrió un error inesperado",
    "WelcomeEmailSubject" => language == "en" ? "Confirm your account" : "Confirma tu dirección de correo",
