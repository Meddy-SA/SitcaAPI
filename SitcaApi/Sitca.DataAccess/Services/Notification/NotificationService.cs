...
public class NotificationService : INotificationService
{
        ...
        Empresa empresa)
    {
        ...
        try
        {
            ...
            var empresaRole = await _db.Roles
              .AsNoTracking()
              .FirstAsync(s => s.Name == "Empresa");

            // Obtener grupos de notificación excluyendo el rol de empresa
            var notificationGroups = notifData.NotificationGroups
                    .Where(s => s.RoleId != empresaRole.Id)
                    .ToList();
            ...
            var cuentasEspecialesVm = cuentasEspeciales.Select(cuenta => new UsersListVm
            {
                Email = cuenta.Email,
